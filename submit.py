...
################################
	...
	X = np.transpose(X)
	a = khatri_rao(X,X)
	X_new = np.transpose(khatri_rao(a,X))
...
################################
	...
	W=np.zeros(729)
	f_vec = get_features(X)
	label_only = get_renamed_labels(y)
...
################################
	...
	#stochastic grad descent 
		...
		c=0.0001
		rndm = rnd.randint(0,n-1)      
		label = label_only[rndm]
		d_point = f_vec[rndm]

		if label*np.dot(d_point,W)  >= 1 :        
			grdnt = W*c                             
		else :
			grdnt = W*c - d_point*label            
